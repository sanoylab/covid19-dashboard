let apiGlobalData = async () => {
  let response = await fetch("https://disease.sh/v3/covid-19/all"); //api end point
...
apiGlobalData()
  .then((res) => {
    ...
    document.getElementById(
      "cases"
    ).innerHTML = global_data.cases
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    document.getElementById(
      "deaths"
    ).innerHTML = global_data.deaths
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    document.getElementById(
      "recovered"
    ).innerHTML = global_data.recovered
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    document.getElementById(
      "active"
    ).innerHTML = global_data.active
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  })
  .catch();
...
let countryDetail = (
  ...
  apiCountryMapData()
    .then((res) => {
      ...
      document.getElementById(
        "active-menu"
      ).innerHTML = `<em class="fa fa-home"></em> <a href="index.html">Global</a>`;
      ...
      document.getElementById(
        "recovered"
      ).innerHTML = recover.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      document.getElementById(
        "active"
      ).innerHTML = active_cases
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");

      document.getElementById(
        "countryNameTitle"
      ).innerHTML = `<img src="${flag}" style="width: 25px; height: 25px; border-radius: 50%"> ${countryName}`;
  ...
  apiCountryMapData()
    .then((res) => {
      ...
      if (countryNameInfo === "") {
        ...
        document.getElementById(
          "cases"
        ).innerHTML = parameter[6]
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById(
          "deaths"
        ).innerHTML = parameter[1]
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById(
          "recovered"
        ).innerHTML = parameter[2]
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById(
          "active"
        ).innerHTML = parameter[3]
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");

        document.getElementById(
          "countryNameTitle"
        ).innerHTML = `<img src="${parameter[7].toString()}" style="width: 25px; height: 25px; border-radius: 50%"> ${parameter[0].toString()}`;
...
let tableDetail = (
  ...
  totalCaseDetail.textContent = totalCase
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  activeDetail.textContent = active
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  recoveredDetail.textContent = recovered
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  deathDetail.textContent = death
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  newCaseDetail.textContent =
    "+" + newCase.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  newDeathDetail.textContent =
    "+" + newDeath.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  totalTestDetail.textContent = totalTest
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
};
...
let apiNewsData = async () => {
  let response = await fetch(
    "https://newscatcher.p.rapidapi.com/v1/search?media=True&sort_by=relevancy&lang=en&page=1&q=covid-19",
    {
      method: "GET",
      headers: {
...
let getCountryChartData = (country) => {
  ...
  apiHistoricalCountryData()
    .then((res) => {
      ...
      barCharts(
        recoveredCountryCategory,
        recoveredCountryValue,
        "case-recovered",
        "#60BB69",
        "Recovered"
      );
...
let barCharts = (chartCategory, chartValues, dom, backColor, title) => {
  ...
  var chart_bar = c3.generate({
    ...
    groups: [chartCategory, chartValues],
    color: {
      pattern: [backColor],
    },
...
let lineCharts = (chartCategory, chartValues, dom) => {
  ...
  d3.select(
    ".c3-legend-item .c3-legend-item-2017 .c3-legend-item-2018 .c3-legend-item-2019"
  )
    .style("float", "right")
